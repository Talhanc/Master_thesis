...
class ESKF {
   public:
    ...
    std::pair<nominal_state, error_state> imu_update(
    ...
    // @return Updated nominal state and error state
    std::pair<nominal_state, error_state> dvl_update(
   ...
   private:
    ...
    error_state current_error_state_;

    // Member variable for the current nominal state
    nominal_state current_nom_state_;
};
